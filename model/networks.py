...
def define_G(opt):
    ...
    if model_opt['netDim'] == 2:
        ...
        from .diffusion_net_3D import diffusion, swinunet
        from .deformation_net_3D import registUnetBlock
    else:
        raise('model dimension error')

    model_score= swinunet.SwinUnet(opt['model']['swin'])

    model_field = registUnetBlock(model_opt['field']['input_size'],
                           model_opt['field']['in_channel'],
                           model_opt['field']['encoder_nc'],
                           model_opt['field']['decoder_nc'],
                           model_opt['field']['int_steps'],
                           model_opt['field']['int_downsize'],
                           model_opt['field']['registration'])
    ...
    if opt['gpu_ids']: #opt['distributed']:
