...
if __name__ == "__main__":
    ...
    if opt['phase'] == 'train':
        ...
        for istep,  test_data in enumerate(test_loader):
            ...
            defm_frames = visuals['contD'].squeeze(1).numpy().transpose(0, 2, 3, 1)
            ...
            print('---- Original Dice: %s | Deformed Dice: %s' % (
            ', '.join(['%.3f' % x for x in vals_origin]), ', '.join(['%.3f' % x for x in vals_regist])))
